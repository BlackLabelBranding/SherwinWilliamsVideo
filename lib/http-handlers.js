...
const { signIn, changeOwnPassword, adminCreateUser, adminResetPassword, setUserActive, deleteUserAccount, listUserProfiles, getUserProfileByUsername } = require('./aws-auth');
const {
  uuidv4,
  allowCognitoAuth,
  commentsByKey,
...
async function handleAuth(request) {
  ...
  if (body.action === 'login') {
    ...
    if (!allowCognitoAuth()) {
      return json(503, {
        ok: false,
        error: 'Cognito is not configured. Set COGNITO_USER_POOL_ID and COGNITO_CLIENT_ID.'
      });
    }
...
async function handleAdminPost(request) {
  ...
  if (body.action === 'create-user') {
    ...
    if (!allowCognitoAuth()) {
      return json(503, { ok: false, error: 'Cognito is not configured.' });
    }
  ...
  if (body.action === 'delete-user') {
    const userId = String(body.userId || body.id || body.username || '').trim();
    if (!userId) {
      return json(400, { ok: false, error: 'User id is required.' });
    }
    if (userId === auth.user.username || userId === auth.user.id) {
