...
function mapDynamoError(error) {
  ...
  if (name === 'ValidationException' || /key element does not match the schema/i.test(msg)) {
    const mapped = new Error(
      `DynamoDB table "${usersTableName()}" key mismatch. Partition key must be "${usersPkName()}" (String).`
    );
    mapped.status = 500;
    return mapped;
  }
  if (name === 'ResourceNotFoundException') {
    const mapped = new Error(`DynamoDB table "${usersTableName()}" was not found.`);
    mapped.status = 500;
    return mapped;
  }
  return error;
...
function mapCognitoError(error) {
  ...
  if (/SECRET_HASH/i.test(msg) || (/configured with secret/i.test(msg) && /SECRET_HASH|not received/i.test(msg))) {
    mapped.message = 'This Cognito app client has a client secret. Create a new app client with “Don’t generate a client secret”, then update COGNITO_CLIENT_ID (or set COGNITO_CLIENT_SECRET if you still have the secret).';
    mapped.status = 500;
...
async function deleteUserAccount(username) {
  ...
  const cognitoUsername = profile?.username || username;
  const userId = String(profile?.user_id || username);
  ...
  try {
    ...
    if (name === 'UserNotFoundException') {
      // already gone from Cognito — still remove DynamoDB row
    } else if (name === 'AccessDeniedException' || /not authorized/i.test(error.message || '')) {
      console.warn('Cognito delete skipped (IAM):', error.message);
    } else {
      throw mapCognitoError(error);
    }
  }
  ...
  try {
    await ddbClient().send(new DeleteCommand({
      ...
      Key: { [usersPkName()]: userId }
    }));
  } catch (error) {
    throw mapDynamoError(error);
  }

  return { ok: true, username: cognitoUsername, user_id: userId };
}
