const crypto = require('crypto');
const {
  CognitoIdentityProviderClient,
  InitiateAuthCommand,
  AdminCreateUserCommand,
  AdminSetUserPasswordCommand,
  AdminDisableUserCommand,
  AdminEnableUserCommand
} = require('@aws-sdk/client-cognito-identity-provider');
const { DynamoDBClient } = require('@aws-sdk/client-dynamodb');
const {
  DynamoDBDocumentClient,
  GetCommand,
  PutCommand,
  ScanCommand
} = require('@aws-sdk/lib-dynamodb');

function isCognitoEnabled() {
  return Boolean(process.env.COGNITO_USER_POOL_ID)
    && Boolean(process.env.COGNITO_CLIENT_ID);
}

function awsRegion() {
  return process.env.AWS_REGION || 'us-east-1';
}

function userPoolId() {
  return process.env.COGNITO_USER_POOL_ID;
}

function cognitoClient() {
  return new CognitoIdentityProviderClient({ region: awsRegion() });
}

function ddbClient() {
  return DynamoDBDocumentClient.from(new DynamoDBClient({ region: awsRegion() }));
}

function usersTableName() {
  return process.env.DDB_USERS_TABLE || 'sw_users';
}

/** Partition key attribute name for sw_users (your table uses user_id). */
function usersPkName() {
  return process.env.DDB_USERS_PK || 'user_id';
}

function mapDynamoError(error) {
  const name = error.name || error.__type || '';
  const msg = String(error.message || '');
  if (name === 'ValidationException' || /key element does not match the schema/i.test(msg)) {
    const mapped = new Error('User profile could not be saved. Please try again.');
    mapped.status = 500;
    return mapped;
  }
  if (name === 'ResourceNotFoundException') {
    const mapped = new Error('User directory is not available. Please contact your administrator.');
    mapped.status = 500;
    return mapped;
  }
  return error;
}

function secretHash(username) {
  const secret = process.env.COGNITO_CLIENT_SECRET;
  const clientId = process.env.COGNITO_CLIENT_ID;
  if (!secret || !clientId) return undefined;
  return crypto
    .createHmac('sha256', secret)
    .update(`${username}${clientId}`)
    .digest('base64');
}

function mapCognitoError(error) {
  const name = error.name || error.__type || '';
  const msg = String(error.message || '');
  const mapped = new Error(msg || 'Authentication failed');
  mapped.status = 400;

  if (/SECRET_HASH/i.test(msg) || (/configured with secret/i.test(msg) && /SECRET_HASH|not received/i.test(msg))) {
    mapped.message = 'Sign-in is misconfigured. Please contact your administrator.';
    mapped.status = 500;
  } else if (name === 'NotAuthorizedException' || name === 'UserNotFoundException') {
    mapped.message = 'Invalid username or password';
    mapped.status = 401;
  } else if (name === 'UserNotConfirmedException') {
    mapped.message = 'This account is not confirmed yet.';
    mapped.status = 403;
  } else if (name === 'PasswordResetRequiredException') {
    mapped.message = 'Password reset is required for this account.';
    mapped.status = 403;
  } else if (name === 'UsernameExistsException') {
    mapped.message = 'Username already exists.';
    mapped.status = 400;
  } else if (name === 'InvalidPasswordException') {
    mapped.message = 'Password does not meet the requirements.';
    mapped.status = 400;
  } else if (name === 'InvalidParameterException' && /USER_PASSWORD_AUTH/i.test(msg)) {
    mapped.message = 'Sign-in is misconfigured. Please contact your administrator.';
    mapped.status = 500;
  } else if (name === 'AccessDeniedException' || /not authorized/i.test(msg)) {
    mapped.message = 'You do not have permission to complete this action.';
    mapped.status = 403;
  } else if (name === 'LimitExceededException' || name === 'TooManyRequestsException') {
    mapped.message = 'Too many attempts. Please wait and try again.';
    mapped.status = 429;
  }
  return mapped;
}

async function getUserProfileById(userId) {
  if (!userId) return null;
  const table = usersTableName();
  const pk = usersPkName();
  try {
    const response = await ddbClient().send(new GetCommand({
      TableName: table,
      Key: { [pk]: String(userId) }
    }));
    return response.Item || null;
  } catch (error) {
    throw mapDynamoError(error);
  }
}

async function getUserProfileByUsername(username) {
  if (!username) return null;
  const exact = String(username).trim();
  const normalized = exact.toLowerCase();

  // Convention: user_id === username for Cognito portal accounts
  const byId = (await getUserProfileById(exact)) || (normalized !== exact ? await getUserProfileById(normalized) : null);
  if (byId) return byId;

  const table = usersTableName();
  try {
    const response = await ddbClient().send(new ScanCommand({
      TableName: table,
      Limit: 200
    }));
    const items = response.Items || [];
    return (
      items.find((item) => String(item.username || '') === exact)
      || items.find((item) => String(item.username || '').toLowerCase() === normalized)
      || items.find((item) => String(item.user_id || '').toLowerCase() === normalized)
      || null
    );
  } catch (error) {
    throw mapDynamoError(error);
  }
}

async function upsertUserProfile(profile) {
  const table = usersTableName();
  const pk = usersPkName();
  const existing = profile.username ? await getUserProfileByUsername(profile.username) : null;
  const userId = String(profile.user_id || existing?.user_id || profile.username);
  const record = {
    [pk]: userId,
    username: profile.username || existing?.username || userId,
    user_id: userId,
    display_name: profile.display_name || existing?.display_name || profile.username || userId,
    role: profile.role || existing?.role || 'driver',
    active: profile.active != null ? Boolean(profile.active) : existing?.active !== false,
    employee_id: profile.employee_id ?? existing?.employee_id ?? '',
    must_change_password: profile.must_change_password != null
      ? Boolean(profile.must_change_password)
      : Boolean(existing?.must_change_password),
    last_login_at: profile.last_login_at || existing?.last_login_at || null,
    created_at: existing?.created_at || profile.created_at || new Date().toISOString(),
    updated_at: new Date().toISOString()
  };
  try {
    await ddbClient().send(new PutCommand({ TableName: table, Item: record }));
  } catch (error) {
    throw mapDynamoError(error);
  }
  return record;
}

async function listUserProfiles() {
  const table = usersTableName();
  try {
    const response = await ddbClient().send(new ScanCommand({ TableName: table }));
    return (response.Items || []).sort((a, b) => String(a.display_name || '').localeCompare(String(b.display_name || '')));
  } catch (error) {
    throw mapDynamoError(error);
  }
}

function placeholderEmail(username) {
  const safe = String(username || 'driver').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 48) || 'driver';
  return `${safe}@drivers.sherwin.local`;
}

function isUsernameExistsError(error) {
  const name = error?.name || error?.__type || '';
  const msg = String(error?.message || '');
  return /UsernameExistsException/i.test(name) || /already exists/i.test(msg);
}

async function adminCreateUser({ username, password, displayName, employeeId, role = 'driver', email = '' }) {
  const client = cognitoClient();
  const pool = userPoolId();
  username = String(username || '').trim().toLowerCase();
  const mail = email || placeholderEmail(username);

  const baseAttrs = [
    { Name: 'name', Value: displayName || username },
    { Name: 'email', Value: mail },
    { Name: 'email_verified', Value: 'true' }
  ];
  const withEmployee = employeeId
    ? [...baseAttrs, { Name: 'custom:employee_id', Value: String(employeeId) }]
    : baseAttrs;

  async function create(attributes) {
    await client.send(new AdminCreateUserCommand({
      UserPoolId: pool,
      Username: username,
      TemporaryPassword: password,
      MessageAction: 'SUPPRESS',
      UserAttributes: attributes
    }));
  }

  try {
    try {
      await create(withEmployee);
    } catch (error) {
      if (error.name === 'InvalidParameterException' && /employee_id|custom:/i.test(error.message || '')) {
        try {
          await create(baseAttrs);
        } catch (retryError) {
          if (!isUsernameExistsError(retryError)) throw retryError;
        }
      } else if (!isUsernameExistsError(error)) {
        throw error;
      }
    }

    await client.send(new AdminSetUserPasswordCommand({
      UserPoolId: pool,
      Username: username,
      Password: password,
      Permanent: true
    }));
  } catch (error) {
    throw mapCognitoError(error);
  }

  return upsertUserProfile({
    username,
    user_id: username,
    display_name: displayName || username,
    role,
    employee_id: employeeId || '',
    active: true,
    must_change_password: true
  });
}

async function adminResetPassword(username, password) {
  try {
    await cognitoClient().send(new AdminSetUserPasswordCommand({
      UserPoolId: userPoolId(),
      Username: username,
      Password: password,
      Permanent: true
    }));
  } catch (error) {
    throw mapCognitoError(error);
  }
  const profile = await getUserProfileByUsername(username) || { username };
  return upsertUserProfile({ ...profile, username, must_change_password: true, active: profile.active !== false });
}

async function setUserActive(username, active) {
  const normalized = String(username || '').trim();
  if (!normalized) {
    const error = new Error('Username is required.');
    error.status = 400;
    throw error;
  }

  let cognitoWarning = null;
  try {
    if (active) {
      await cognitoClient().send(new AdminEnableUserCommand({
        UserPoolId: userPoolId(),
        Username: normalized
      }));
    } else {
      await cognitoClient().send(new AdminDisableUserCommand({
        UserPoolId: userPoolId(),
        Username: normalized
      }));
    }
  } catch (error) {
    const name = error.name || error.__type || '';
    // IAM may lack AdminEnableUser/AdminDisableUser — still enforce via DynamoDB active flag on login.
    if (name === 'AccessDeniedException' || /not authorized/i.test(error.message || '')) {
      cognitoWarning = error.message;
      console.warn('Cognito enable/disable skipped (IAM):', error.message);
    } else if (name === 'UserNotFoundException' && normalized !== normalized.toLowerCase()) {
      return setUserActive(normalized.toLowerCase(), active);
    } else if (name === 'UserNotFoundException' && normalized === normalized.toLowerCase()) {
      // Dynamo-only enforcement if Cognito user key differs by case.
      console.warn('Cognito user not found for enable/disable:', normalized);
    } else {
      throw mapCognitoError(error);
    }
  }

  const profile = await getUserProfileByUsername(normalized) || { username: normalized };
  const saved = await upsertUserProfile({ ...profile, username: profile.username || normalized, active: Boolean(active) });
  if (cognitoWarning) {
    saved.cognito_sync = 'skipped_iam';
  }
  return saved;
}

async function signIn({ username, password }) {
  username = String(username || '').trim().toLowerCase();
  const client = cognitoClient();
  const hash = secretHash(username);
  const authParameters = {
    USERNAME: username,
    PASSWORD: password
  };
  if (hash) authParameters.SECRET_HASH = hash;

  let auth;
  try {
    auth = await client.send(new InitiateAuthCommand({
      ClientId: process.env.COGNITO_CLIENT_ID,
      AuthFlow: 'USER_PASSWORD_AUTH',
      AuthParameters: authParameters
    }));
  } catch (error) {
    throw mapCognitoError(error);
  }

  if (!auth.AuthenticationResult?.AccessToken) {
    const error = new Error('Additional sign-in steps are required for this account. Please contact your administrator.');
    error.status = 400;
    throw error;
  }

  let profile = await getUserProfileByUsername(username);
  if (!profile) {
    profile = await upsertUserProfile({
      username,
      user_id: username,
      display_name: username,
      role: username === 'admin' ? 'admin' : 'driver',
      active: true
    });
  } else if (username === 'admin' && profile.role !== 'admin') {
    // Repair profiles created by a case-mismatched first login (e.g. "Admin").
    profile = await upsertUserProfile({
      ...profile,
      username: 'admin',
      role: 'admin'
    });
  }
  if (!profile.active) {
    const error = new Error('Account is disabled.');
    error.status = 403;
    throw error;
  }

  profile = await upsertUserProfile({
    ...profile,
    last_login_at: new Date().toISOString()
  });

  return {
    ok: true,
    accessToken: auth.AuthenticationResult.AccessToken,
    idToken: auth.AuthenticationResult.IdToken || null,
    refreshToken: auth.AuthenticationResult.RefreshToken || null,
    profile
  };
}

async function changeOwnPassword({ username, currentPassword, newPassword }) {
  // Verify the current password, then set the new one server-side.
  // Avoid Cognito ChangePasswordCommand (needs access token / challenge edge cases).
  try {
    await signIn({ username, password: currentPassword });
  } catch (error) {
    if (error.status === 403 && /disabled/i.test(error.message || '')) throw error;
    const err = new Error('Current password is incorrect.');
    err.status = 401;
    throw err;
  }

  try {
    await cognitoClient().send(new AdminSetUserPasswordCommand({
      UserPoolId: userPoolId(),
      Username: username,
      Password: newPassword,
      Permanent: true
    }));
  } catch (error) {
    throw mapCognitoError(error);
  }

  const profile = await getUserProfileByUsername(username) || { username };
  return upsertUserProfile({ ...profile, username, must_change_password: false });
}

module.exports = {
  isCognitoEnabled,
  signIn,
  changeOwnPassword,
  adminCreateUser,
  adminResetPassword,
  setUserActive,
  getUserProfileByUsername,
  upsertUserProfile,
  listUserProfiles
};
