...
function pickBestVariantUri(playlistText) {
...
function safeDownloadFilename(title, fallbackKey) {
  const fromTitle = String(title || '')
    .replace(/[^\w\s.-]+/g, '')
    .trim()
    .replace(/\s+/g, '-')
    .slice(0, 80);
  if (fromTitle) return `${fromTitle}.ts`;
  const fromKey = String(fallbackKey || 'recording')
    .split('/')
    .filter(Boolean)
    .slice(-4, -3)[0] || 'recording';
  return `${fromKey}.ts`;
}
...
async function resolveMediaPlaylist(client, bucket, playlistKey) {
  let key = assertAllowedKey(playlistKey);
  let text = await getObjectText(client, bucket, key);
  const variantUri = pickBestVariantUri(text);
  if (variantUri) {
    if (/^https?:\/\//i.test(variantUri)) {
      const error = new Error('Absolute variant playlists are not supported for download.');
      error.status = 400;
      throw error;
    }
    key = assertAllowedKey(resolveKey(key, variantUri));
    text = await getObjectText(client, bucket, key);
  }
  return { key, text };
}

function mediaSegmentRefs(playlistText, playlistKey) {
  const refs = [];
  for (const line of String(playlistText || '').split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith('#')) continue;
    if (/^https?:\/\//i.test(trimmed)) {
      refs.push({ type: 'url', value: trimmed });
    } else {
      refs.push({ type: 'key', value: resolveKey(playlistKey, trimmed) });
    }
  }
  return refs;
}

async function buildArchiveDownload(playlistKey, downloadTitle) {
  const client = createS3Client();
  const bucket = process.env.S3_BUCKET;
  if (!client || !bucket) {
    const error = new Error('S3 is not configured.');
  ...
  const media = await resolveMediaPlaylist(client, bucket, playlistKey);
  const refs = mediaSegmentRefs(media.text, media.key);
  if (!refs.length) {
    const error = new Error('No video segments found for download.');
    error.status = 404;
    throw error;
  }

  const filename = safeDownloadFilename(downloadTitle || titleFromKey(playlistKey), playlistKey);
  const stream = new ReadableStream({
    async start(controller) {
      try {
        for (const ref of refs) {
          if (ref.type === 'key') {
            controller.enqueue(await getObjectBytes(client, bucket, ref.value));
            continue;
          }
