const { listArchiveRecordings, buildPresignedPlaylist, trimAndReplacePlaylist, deleteArchiveRecording, buildArchiveDownload, ensureBucketCors } = require('./s3-archive');
const { listActiveLiveStreams, pickLiveEvent } = require('./ivs-live');
const viewingMetrics = require('./viewing-metrics');
...
async function refreshLiveFromIvs() {
  // A failed status lookup is not an offline broadcast. The IVS helper
  // supplies a bounded last-known-good snapshot or throws a retryable 503.
  // Existing clients ignore failed refreshes, leaving the player mounted.
...
async function handleContentGet(request) {
  ...
  try {
    ...
    return json(error.status || 500, { ok: false, error: error.message || 'Failed to load archive from S3.' });
  }
}
...
async function handleDownload(request) {
  ...
  try {
    const file = await buildArchiveDownload(String(key || ''), name || '');
    return {
      status: 200,
      body: file.stream,
    ...
    const status = error.status || error.$metadata?.httpStatusCode || 500;
    return json(status, { ok: false, error: error.message || 'Failed to download recording.' });
  }
}
...
async function handleMetrics(request) {
  ...
  try {
    if (body.action === 'start') {
      return json(200, await viewingMetrics.startSession(auth.user, body));
    }
    if (body.action === 'heartbeat' || body.action === 'end') {
      return json(200, await viewingMetrics.updateSession(auth.user, body));
    }
    if (body.action === 'dashboard') {
      if (auth.user.role !== 'admin') {
        return json(403, { ok: false, error: 'Admin access required' });
      }
      return json(200, await viewingMetrics.report(body.date || viewingMetrics.chicagoDay()));
    }
  } catch (error) {
    console.warn('Viewing analytics unavailable:', error.name, error.message);
    return json(error.status || 503, { ok: false, error: 'Attendance tracking/reporting is unavailable. Video playback is unaffected.' });
  }
...
async function handleAdminGet(request) {
  ...
  try {
    ...
    if (allowCognitoAuth()) {
      try {
        userRows = (await listUserProfiles())
          .filter((u) => {
            const username = String(u.username || '').toLowerCase();
            const display = String(u.display_name || '').toLowerCase();
            return !username.includes('duplicate-removed') && display !== 'removed duplicate';
          })
          .map((u) => ({
          ...u,
...
async function handleAdminPost(request) {
  ...
  if (body.action === 'set-user-active') {
    ...
    const selfId = String(auth.user.id || '').trim().toLowerCase();
    const selfUsername = String(auth.user.username || '').trim().toLowerCase();
    const isSelf = (selfId && target === selfId) || (!selfId && target === selfUsername);
    if (body.active === false && isSelf) {
      return json(400, { ok: false, error: 'You cannot disable your own account.' });
    }
    try {
