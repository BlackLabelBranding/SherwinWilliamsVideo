...
function mapCognitoError(error) {
  ...
  if (name === 'NotAuthorizedException' || name === 'UserNotFoundException') {
    ...
    mapped.status = 500;
  } else if (/SECRET_HASH/i.test(msg) || (/secret/i.test(msg) && /not received|configured with secret/i.test(msg))) {
    mapped.message = 'Cognito client secret is required. Set COGNITO_CLIENT_SECRET in .env / Vercel.';
    mapped.status = 500;
